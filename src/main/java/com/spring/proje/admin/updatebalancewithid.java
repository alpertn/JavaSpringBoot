...
import com.spring.proje.admindto.updatebalancewithiddto;
...
public class updatebalancewithid {
    ...
    public ResponseEntity<?> request(@Valid
        ...
        if (sqlinjectiontester.check(request.getadminname() + request.getadminpassword()+ request.getid()+ request.getbalance().toString()) == true)
        {
            ...
            if(bindingresult.hasErrors()){
                ...
                if (checkadmin.checkadmin(request.getadminname(),request.getadminpassword()) == true){
                    endpointlogger.log(HttpServletRequest, request.getadminname(),request.getadminpassword(), "/api/admin/updatebalancewithid");
